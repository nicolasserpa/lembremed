window.AgendaLogic = {
  calculateDailyProgress: function(medsArray) {
    ...
    const validMeds = medsArray.filter(m => m.status !== 'cancelado' && m.status !== 'ignorado');
    const total = validMeds.length;

    if (total === 0) return { taken: 0, total: 0, progress: 0 };

    const taken = validMeds.filter(m => m.status === 'tomado').length;
